...
const SizedBox(height: 100),
          ...
          // 🟢 核心展示区：PK Timer 和 Score Bar 的组合层叠
          Padding(
                ...
                Wrap(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      ...
                      child: const Text("🔥 倒计时 10s (触发狂暴状态)"),
                    ),
                    ElevatedButton(
                      style: ElevatedButton.styleFrom(backgroundColor: Colors.red[800]),
                      onPressed: () => setState(() => myScore += 1000000),
                      child: const Text("我方 +100万 (高分打爆)"),
                    ),
                    ElevatedButton(
                      style: ElevatedButton.styleFrom(backgroundColor: Colors.orange),
                      onPressed: () => _triggerCrit(true),
                      child: const Text("我方触发暴击"),
                    ),
                    ElevatedButton(
...
if (widget.status == PKStatus.idle) return const SizedBox();
    ...
    if (isHighScore) {
      ...
      if (diff > 0) myScoreText = "领先 $diffStr";
      else if (diff < 0) myScoreText = "落后 $diffStr";
      else myScoreText = "平局";
      oppScoreText = "";
    }
            ...
            return Stack(
              ...
              children: [
                Column(
                  ...
                  children: [
                    // 🟢 核心变宽：使用 AnimatedBuilder 监听 _feverController
                    AnimatedBuilder(
                      animation: _feverController,
                      builder: (context, child) {
                        // 高度在最后10秒会从 18 爆弹到 32
                  ...
                  Positioned(
                    ...
                    child: AnimatedBuilder(
                      ...
                      child: Image.network(
                        ...
                        width: 28, height: 28, colorBlendMode: BlendMode.multiply,
                      ),
                    ),
                  ),
              ],
...
// 🛠️ 狂暴岩浆碎片 - 核心参数调节区
// ================================================================
final double safeOffset = 15.0;    // 📍 距离箭头尾部的空白距离
final double spawnRate = 0.5;      // 🔢 生成密集度
final double speedMin = 20.0;      // 🚀 最小流速 (如果你觉得跑得太快了，把这两个值改小！)
final double speedMax = 50.0;      // 🚀 最大流速
final double jumpPower = 30.0;     // 💥 【散射幅度】：调大这个值，它们飞得更散。

// 🟢 调整了光晕的大小，让它看起来更厚重、有质感
final double sizeMin = 2.0;        // 🟡 光晕核心最小尺寸
final double sizeMax = 5.0;        // 🟡 光晕核心最大尺寸
final double lifeTime = 0.5;       // ⏳ 存活时间
// ================================================================
...
final double vy = _random.nextDouble() * 4 - 2;
    ...
    return _PKParticle(
      ...
      size: _random.nextDouble() * 1.0 + 0.5, color: Colors.white,
      life: 1.0, decayRate: _random.nextDouble() * 1.2 + 0.6,
    );
}

_PKFeverFragment _createFeverFragment() {
// 💡 核心控制：碎片的生成起点也要跟随箭头的锐利度后退，防止穿模
...
canvas.clipRect(Rect.fromLTRB(-500, 0, size.width + 500, size.height));
    ...
    if (feverValue == 0) {
      if (isZeroScore) {
        final glowPaint = Paint()..color = Colors.white.withOpacity(0.9)..strokeWidth = 3.0..maskFilter = const MaskFilter.blur(BlurStyle.normal, 2.5);
        final corePaint = Paint()..color = Colors.white..strokeWidth = 1.5;
      ...
      // 2. ⭐ 终极重构：绘制炽热连绵的高温光晕 (Glow/Halo Effect)
      canvas.save();
      // ⭐ 核心：使用 clipPath，把所有光晕限制在光束范围内，确保没有生硬的竖线截断感
      ...
      for (var f in feverFragments) {
        ...
        // ⭐ 核心修复：光晕画笔。把 MaskFilter.blur 加回来，制造完美的球形光晕。
        // 强力模糊 (sigma: 3.5)，让斑块边界完全消溶在血条里。
        final haloPaint = Paint()
          ..color = spotColor.withOpacity(opacity * 0.95)
          ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 3.5);

        // ⭐ 核心修复：从 Path 变回 Circle。圆形的强模糊才是最自然的光晕。
        canvas.drawCircle(
          Offset(centerX + f.x, centerY + f.y),
          currentRadius,
          haloPaint,
        );
      }
    ...
    for (var p in particles) {
      final paint = Paint()..color = p.color.withOpacity(p.life.clamp(0.0, 1.0))..style = PaintingStyle.fill..maskFilter = const MaskFilter.blur(BlurStyle.solid, 0.5);
